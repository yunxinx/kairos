...
test.describe('channel resource page', () => {
  test('creates, edits, probes, and deletes a channel', async ({ page }) => {
    ...
    try {
      ...
      await page.locator('[id^="channel-editor-priority"]').fill('10');
      await page.locator('[id^="channel-editor-weight"]').fill('2');
      ...
      await expect(page.locator('[id^="channel-editor-priority"]')).toHaveValue('10');
      await expect(page.locator('[id^="channel-editor-weight"]')).toHaveValue('2');
