...
/// 把「USD / 1M tokens」浮点价换算为「/ 1M tokens 的整数 micro-USD」。
///
/// 换算发生在配置边界，之后金额一律整数运算。
fn micros_per_1m(usd_per_1m: f64) -> i64 {
    (usd_per_1m * 1_000_000.0).round() as i64
}
...
impl PriceSnapshot {
    /// 从配置价格构造快照；缓存档缺省时回退 `input` 价。
    pub fn from_price(price: &Price) -> Self {
        let input = micros_per_1m(price.input);
        let output = micros_per_1m(price.output);
        Self {
            input_micros: input,
            output_micros: output,
            cache_read_micros: price.cache_read.map(micros_per_1m).unwrap_or(input),
            cache_write_micros: price.cache_write.map(micros_per_1m).unwrap_or(input),
        }
    }
}
...
mod tests {
    ...
    fn cost_charges_each_dimension_by_its_price() {
        let price = PriceSnapshot::from_price(&Price {
            input: 2.5,
            output: 10.0,
    ...
    /// 快照构造不依赖 JSON 序列化，且价格取整确定。
    #[test]
    fn snapshot_is_deterministic() {
        let a = PriceSnapshot::from_price(&Price {
            input: 2.5,
            output: 10.0,
            cache_read: Some(1.25),
            cache_write: Some(10.0),
        });
        let b = PriceSnapshot::from_price(&Price {
            input: 2.5,
            output: 10.0,
            cache_read: Some(1.25),
            cache_write: Some(10.0),
