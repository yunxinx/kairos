//! 渠道密钥模型与选择策略。
//!
//! 管理 API 使用 [`ChannelKey`] 作为明文 wire DTO；数据库加载后的运行时密钥使用
//! [`StoredChannelKey`]，其中明文被 `secrecy` 包裹。选择策略集中在本模块，探测与
//! 会话粘性路由共享同一套候选过滤和加权规则。
...
impl StoredChannelKey {
    ...
    pub fn to_wire(&self) -> ChannelKey {
        ChannelKey {
            ...
            api_key: self.expose_api_key().to_string(),
            weight: self.weight,
